...
async function createInfinitePayCheckout(sub: any) {
  const orderNsu = crypto.randomUUID();
  const webhookUrl = `${SUPABASE_URL}/functions/v1/payment-webhook${WEBHOOK_SECRET ? `?secret=${encodeURIComponent(WEBHOOK_SECRET)}` : ""}`;
  const redirectUrl = PUBLIC_SITE_URL ? `${PUBLIC_SITE_URL}/login.html?renovacao=infinitepay&order_nsu=${encodeURIComponent(orderNsu)}` : undefined;
    ...
    items: [
      {
        ...
        description: `${sub.plan_label || sub.plan_name || "Plano"} - Renovação NavalhaOS (${sub.plan_display_price || ""})`,
      },
    ],
    order_nsu: orderNsu,
  ...
  if (!linkRes.ok) {
    throw new Error(linkJson?.message || linkJson?.error || "Erro ao criar link de renovação.");
  }
...
serve(async (req) => {
  ...
  try {
    ...
    for (const sub of dueSubs || []) {
      if (sub.renewal_created_at && sub.status === "renewal_pending") continue;

      const checkout = await createInfinitePayCheckout(sub);
    ...
    return jsonResponse({
      ...
      renewalsCreated,
      accountsBlocked,
      message: "Renovações processadas.",
    });
  } catch (err) {
